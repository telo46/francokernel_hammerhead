...
static inline cputime64_t get_cpu_idle_time_old(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(od_cpu_dbs_info, j);
		dbs_info->prev_cpu_idle = get_cpu_idle_time_old(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time_old(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	if (policy->cur < policy->max) {
		...
		this_dbs_info->prev_cpu_idle = get_cpu_idle_time_old(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time_old(j,
